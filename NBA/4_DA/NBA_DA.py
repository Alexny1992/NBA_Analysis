...
def getTop12():
    ...
    Return: Dataframe 
    """
    df_players = pd.read_csv('2013-2023-Player-edit.csv')
...
def hist_SeasonPlayerPER():
    ...
    for i, num  in zip(range(2012, 2023), range(1, 22)):
...
def qq_SeasonPlayerPER():
    ...
    title_list = []
    for i in range(2012, 2023): # to make title list 
...
def teamRatio_calc():
    ...
    returns: CSV file
    """
    df_teams = pd.read_csv('2013-2023-Regular-TeamTotals-edit.csv', index_col=0)

    team_tm = {
            'Atlanta Hawks': 'ATL',
            'Boston Celtics': 'BOS',
            'Brooklyn Nets': 'BRK',
            'Charlotte Hornets': ['CHH', 'CHA'],
            'Chicago Bulls': 'CHI',
